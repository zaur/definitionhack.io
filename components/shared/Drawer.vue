<template>
  <div class='drawer' :class='{ finished: isFinished }'>
    <div ref='drawer' />
  </div>
</template>
...
export default {
  props: {
    quantity: { type: [Number, String], default: 10 },
  },
  ...
  computed: {
    speed () {
      return this.time / this.finalPath.length
    }
  },
  ...
  methods: {
    calcPath () {
      ...
      const fraction = Number(this.quantity) / parseFloat('256')
      this.finalPath = this.path.filter(() => random() < fraction);
    },
    ...
    draw () {
      ...
      this.calcPath()
      this.$nextTick(() => {
        const diagram = new FourierDiagram (this.$refs.drawer, this.finalPath, this.speed);
        diagram.draw()
      })
      this.finalDraw();
    }
